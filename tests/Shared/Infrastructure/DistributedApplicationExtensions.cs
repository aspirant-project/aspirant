...
public static partial class DistributedApplicationExtensions
{
    ...
    {
        var contentRootPath = builder.Configuration["AppHost:Directory"] ?? throw new InvalidOperationException("Missing expected configuration value 'AppHost:Directory'.");
        var hostEnvironment = builder.Services.First(sd => sd.ServiceType == typeof(IHostEnvironment)).ImplementationInstance as IHostEnvironment
            ?? throw new InvalidOperationException("Service descriptor for IHostEnvironment could not be found.");
        hostEnvironment.ContentRootPath = contentRootPath;
        hostEnvironment.ContentRootFileProvider = new PhysicalFileProvider(contentRootPath);
